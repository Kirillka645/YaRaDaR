...
/**
 * Обоснованный кэф из открытых сигналов (погода, OSM POI, час).
 * Без рекурсии: кэф считается отдельно, прогноз — через coefOnly().
 */
object GroundedDemandModel {
    ...
    fun compute(input: Input): Result {
        ...
        val coef = coefOnly(input)

        val baseOrders = input.profile.baseOrdersPerHour * input.cityScale
        val perHour = max(
            1.0,
            baseOrders * (0.4 + hourF * 0.5 + poi * 0.8 + weather * 0.6 + rush * 0.4)
        )
        ...
        for (h in 0..input.hour) {
            day += baseOrders *
                DemandForecastEngine.hourFactor(h, input.profile, input.weekend) * 0.9
        }
        val pressure = (lastHour / (baseOrders * 1.15).coerceAtLeast(1.0)).coerceIn(0.2, 2.8)
        val orders = DistrictOrderStats(
    ...
    /** Только коэффициент — без forecast (чтобы не было рекурсии). */
    fun coefOnly(input: Input): Double {
        val hourF = DemandForecastEngine.hourFactor(input.hour, input.profile, input.weekend)
        val rush = rushFactor(input.hour, input.weekend)
        val weather = input.weatherBoost.coerceIn(0.0, 0.7)
        val poi = input.poiScore.coerceIn(0.0, 1.0)
        ...
        val demandIndex =
            0.15 * hourF +
                ...
                weather * 0.9 +
                weekendNight
        return round1((1.0 + demandIndex * 0.85).coerceIn(1.0, 2.8))
    }
    ...
    private fun buildForecast(current: Double, input: Input): ZoneForecast {
        fun at(minutes: Int): Double {
            val h = ((input.hour + minutes / 60) % 24).let { if (it < 0) it + 24 else it }
            // coefOnly — без вызова compute/buildForecast
            val future = coefOnly(input.copy(hour = h))
            val a = when {
                minutes <= 15 -> 0.3
                minutes <= 30 -> 0.5
                else -> 0.7
